...
\Macro{KOMAoptions} command as well as some other macros are not available in earlier versions,
...
\Option{BCOR} (set by default to \qty{5}{\mm}),
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{0.8\textwidth}}
    ...
    as \Option{scale=0.75} to cover \qty{75}{\%} of the page.\\
...
Note that if you use \Option{UKenglish} or \Option{USenglish} and the \Package{cleveref} package,
and use \Macro{cref} or \Macro{Cref} in a figure caption to refer to another figure or table,
the cross-reference will be in German in the list of tables/figures.
The only way I have found to get round this problem is to remove any use of \Option{ngerman}
as a language in your thesis or to add \Option{english} as a language to the document class
and to edit the \File{.lof} and \File{.lot} files by adding a line:
\Macro{babel@toc \{english\}\{\}\textbackslash relax} after other such lines
and recompiling once using \Command{pdflatex}  etc.\ and not \Command{latexmk}.
The second is not a nice option, as you have to do this every time the \File{.lof} or \File{.lot} files change,
or remember to do it just before you submit your thesis.
If you want to have a list of figures/tables,
the other alternative is not to use the \Package{cleveref} macros in captions.
...
The package \Package{tabularray} was released in 2021.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{0.8\textwidth}}
    ...
    \Package{axodraw2} & Draw Feynman graphs. \Package{axodraw2} is a newer replacement for \Package{axodraw}.\\
    ...
    \Package{lineno} & Show line numbers.\\
    ...
    \Package{gitver} & Allows you to output information on the Git version used for the document.\\
    ...
    \Package{adjustbox} & Add much more sophisticated clipping capabilities than offered by \Package{graphicx}.\\
...
If you use \TeXLive 2017 or earlier pass the \Option{diffcoeffISO} option to \Package{ubonn-thesis}.
...
\end{tcblisting}
\noindent before \Macro{documentclass}.
The dependencies can be found after running pdf\LaTeX{} (or whichever engine you run)
in the file \File{mythesis.dep} (or whatever your main filename is).
A file \File{mythesis.dep} can be found in the main directory of \File{ubonn-thesis}.
...
If you want to record the Git version used when compiling your thesis
you can try out the \Package{gitver} package.
Pass the option \Option{noheader} to the package.
In order for \Package{gitver} to work, 
you have to pass the option \Option{-shell-escape} to
\texttt{pdflatex} or \texttt{latexmk}.
...
The \Package{subfiles} package has been included at some point
in the \Package{texlive-latex-extra} collection.
It is a convenient way to be able to compile single files in a document.
This can also be done with \Macro{includeonly}, but this requires changes to the document each time.
With \Package{subfiles} you can just run pdf\LaTeX (or whatever engine you are using)
on a single file in your document).
Have a look at \url{http://en.wikibooks.org/wiki/LaTeX/General_Guidelines} for
example for more information.
% If you want to use the package you have
...
The \Package{changes} package is another way of recording changes and adding comments.
It is supposed to work much like change markup in LibreOffice, Microsoft Office etc.

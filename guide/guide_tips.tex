...
I no longer support versions of \TeXLive older than 2015.
...
The style file should work for all recent \TeX\ installations.
% However, some of the packages I recommend have
...
This guide should compile without changes for \TeXLive 2018 or later.
% However, you can get rid of some \Package{diffcoeff} warnings
% by setting the year to 2018 or later.
For earlier versions set \Macro{TweakYear} in \texttt{thesis\_guide.tex} to the appropriate value.
...
\endlastfoot
  ...
  lineno & false & Show line numbers -- only works in \Option{Draft} mode.\\
  linenofix & true & Fix line numbers around AMS math environments without using the \Package{linenomath} environment.\\
...
The \texttt{LaTeX Workshop}\index{LaTeX Workshop@\LaTeX\ Workshop} extension is being actively developed
and is very nice.
...
\Macro{MeVovercsq} to produce \unit{\MeVovercsq} is defined as \(c_{0}\) in
...
\DeclareSIUnit\clight{\text{\ensuremath{c}}}   % siunitx version 2
\DeclareSIUnit\clight{\ensuremath{\mathit{c}}} % siunitx version 3 (after 2021/04/09)
\end{tcblisting}
...
They can be included by passing the option \Option{numerr} to 
\Package{ubonn-thesis}.
...
Rounding\index{rounding} is very nice and works well, but can slow down compilation considerably.
...
Note that the option \Option{numpmcorr} can be used to make the superscript and the subscript
have the same width.
...
This is a very useful package that may be useful when writing your thesis
and occasionally has some problems.
Hence, I thought I would include some information here.
Pass the option \Option{lineno=true} to \Package{ubonn-thesis} to get line numbers.
This will only turn on line numbers when in \Option{Draft} mode.
...
Sometimes a block of lines is not numbered -- see \cref{fig:lineno}a.
Such problems are associated with text that is close to math mode environments.
Most of the problems can be solved by using a new
version of the \Package{lineno} package.
However, while this works for \enquote{standard} \LaTeX{}
math environments: \Env{displaymath}, \Env{equation} and \Env{eqnarray},
it may not work for recommended \Package{amsmath} environments such as \Env{equation*},
\Env{align(*)} and \Env{alignat(*)}. % chktex 36
If the option \Option{linenofix} is set, which is default,
then it should also work for these environments.
If this option causes other problems turn it off and use the solution below.

Enclose the equation in \Env{linenomath} environment, e.g.
\begin{tcblisting}{listing only}
...
\File{ubonn-thesis.sty} and \File{ubonn-biblatex.sty}) are copied
...
The \File{Makefile} is not updated in this process (as of version 9.1),
as changes to it are only rarely made,
and you may have adapted it to your use.
Have a look at \File{thesis\_skel/Makefile} to see if there are any updates to the \File{Makefile} that you should also include.

...
\label{sec:fig}\index{figures}\index{Feynman graphs}
%==============================================================================
...
More details on the use of \Package{TikZ-FeynHand} and my Python wrapper \Package{PyFeynhand}
can be found in \cref{sec:fig:feynman:tikz}.
...
Alternatives include \Package{axodraw2} and the \Package{feynmf} or
\Package{feynmp} packages.

An example using the package \Package{axodraw} is available in \cref{sec:fig:axodraw}.
...
%------------------------------------------------------------------------------
\subsection{Axodraw}%
\label{sec:fig:axodraw}\index{axodraw}

\Cref{fig:axodraw:ppttbar} was given to me by Richard Nisius and illustrates some of the
things that can be done using the \Package{axodraw} package.

\begin{figure}[htbp]
\begin{tcblisting}{}
\centering
\input{../axodraw/pp-tt-btoX}
...
you need to either run \texttt{axohelp file}\index{axohelp} after \texttt{pdflatex file},
or add the following lines to your \texttt{latexmk}\index{latexmk} configuration.
...
See the \Package{axodraw2} manual for more information.

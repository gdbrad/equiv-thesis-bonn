...
\todo{brief exposition on octahedral group and symmetries of the cube }
...
\subsection{Single derivative operators}
...
\end{figure}
The optimized single-meson operators are denoted by $M^{\[n_1n_2n_3]}_\Lambda$ where $M$ is the meson, $\Lambda$ is the lattice irrep, $\[n_1n_2n_3]$ is the momentu\m in units of $\frac{2\pi}{L}$.  \cite{Cheung_2017}\
...
\begin{align}
    O(x_0,t_0) &= \Big\[\sum_{x_1} S_i^{t_0,\alpha_0,a_0}(x_1)^{\alpha_1}_{a_1}\bar{q}^{(f)}(x_1,t)^{\alpha_1}_{a_1}\Big] \Gamma^{\\alpha_0\beta_0} \Big\[\sum_{x_2} (S_k^{t_0,\beta_0,a_0}(x_2)^\dagger)^{\alpha_2}_{a_2}\bar{q}^{(f')}(x_2,t)^{\alpha_2}_{a_2}\Big\] \\
    &= \overline q_i(x_0, t)^{\alpha_0}_{a_0} \Gamma^{\alpha_0\beta_0} q_k(x_0,t)^{\beta_0}_{a_0}
\end{align}

Let $S = \Box$ and Fourier transform into $p$-space. 
$$
\overline \chi(p, t) = \overline u_w(t) \Box_{wx}(t) \cdot e^{-ip\cdot x} \Gamma^A_{xy}(t) \cdot \Box_{yz}(t) c_z(t)
$$
...
\Phi^B(t^\prime) 
\cdot
\underbrace{V^\dagger(t^\prime) c(t^\prime) \overline c(t) V(t)}_{\tau_c(t^\prime, t)}
\cdot
\Phi^A(t)
\cdot
